...
#else
#include "wiimote_state-bootes.h"
#endif
...
class wiimote : public wiimote_state
	...
	private: // methods
		...
		void OnReadData				(DWORD bytes_read, DWORD read_time); //bootes
		// parse individual reports by type
		int	 ParseInput				(BYTE* buff, DWORD read_time); //bootes
		...
		int	 ParseAccel				(BYTE* buff, DWORD read_time); //bootes
		...
		int  ParseExtension			(BYTE* buff, unsigned offset, DWORD read_time); //bootes
