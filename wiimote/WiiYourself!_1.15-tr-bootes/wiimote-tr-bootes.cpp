...
// modified by unknown ( http://dl.dropbox.com/u/14624144/wiimote.cpp )
//   to support wiimote TR model(motionplus bundled version).
//
// modifyed by Daisuke Kanda <dai1975@gmail.com>
//   to add time value at which accel and gyro data is received.
...
unsigned __stdcall wiimote::ReadParseThreadfunc (void* param)
	...
	while(1)
		...
		DWORD read_time = timeGetTime(); //bootes
...
//			   !remote.bConnectInProgress   &&
		...
		if(read) {
			...
			remote.OnReadData(read, read_time);
			}
		else
...
// ------------------------------------------------------------------------------------
void wiimote::OnReadData (DWORD bytes_read, DWORD read_time)
	{
...
// ------------------------------------------------------------------------------------
int wiimote::ParseInput (BYTE* buff, DWORD read_time)
	{
	int changed = 0;
	...
	switch(buff[0])
		...
		case IN_BUTTONS_ACCEL:
			...
			changed |= ParseButtons(buff);
			if(!IsBalanceBoard())
				changed |= ParseAccel(buff, read_time);
			break;

		case IN_BUTTONS_ACCEL_EXT:
			DEEP_TRACE(_T(".. parsing extenion/accel."));
			changed |= ParseButtons(buff);
			changed |= ParseExtension(buff, 6, read_time);
			if(!IsBalanceBoard())
				changed |= ParseAccel(buff, read_time);
			break;
		...
		case IN_BUTTONS_ACCEL_IR_EXT:
			...
			changed |= ParseButtons(buff);
			changed |= ParseExtension(buff, 16, read_time);
			if(!IsBalanceBoard()) {
				changed |= ParseAccel(buff, read_time);
				changed |= ParseIR	   (buff);
				}
		...
		case IN_BUTTONS_BALANCE_BOARD:
			...
			changed |= ParseButtons(buff);
			changed |= ParseExtension(buff, 3, read_time);
			break;
...
// ------------------------------------------------------------------------------------
int wiimote::ParseAccel (BYTE* buff, DWORD read_time)
	{
	int changed = 0;
	...
	Internal.Acceleration.Time = read_time; //bootes
...
// ------------------------------------------------------------------------------------
int wiimote::ParseExtension (BYTE *buff, unsigned offset, DWORD read_time)
	{
	int changed = 0;
...
/*
				float y_scale    =   yaw_slow? 0.07263 : 0.330144f; //bootes: wiibrew says
				float p_scale    = pitch_slow? 0.07263 : 0.330144f; //low scale is 595/8192
				float r_scale    =  roll_slow? 0.07263 : 0.330144f; //high scale is low*(2000/440)
*/			
				Internal.MotionPlus.Time = read_time; //bootes
