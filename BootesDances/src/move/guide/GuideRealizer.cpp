...
std::basic_string< TCHAR > GuideRealizer::GetFilePath(const TCHAR* basepath, const TCHAR* subdir, const TCHAR* guide)
{
   std::basic_ostringstream< TCHAR > o;
   if (subdir && *subdir != _T('\0')) {
      o << basepath << _T("\\") << subdir;
   } else {
      o << basepath;
   }
   o << _T("\\guide-") << guide << _T(".txt");
   return o.str();
}

bool GuideRealizer::IsExist(const TCHAR* basepath, const TCHAR* subdir, const TCHAR* guide)
{
   std::basic_string< TCHAR > path = GetFilePath(basepath, subdir, guide);
...
bool GuideRealizer::load(std::list< GuideData >* pOut, const TCHAR* basepath, const TCHAR* subdir) const
{
   int fd = -1;
   std::basic_string< TCHAR > path = GetFilePath(basepath, subdir, getGuideNameT());
