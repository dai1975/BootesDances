...
MoveModelSpline::MoveModelSpline()
{
...
MoveModelSpline::MoveModelSpline(const MoveModelSpline& r)
: MoveModel(r)
{
}
...
void MoveModelSpline::init(const t_points& points)
{
   if (points.size() < 3) {
      _edit_points[0] = D3DXVECTOR3(0.2f, 0.8f, 0.0f);
      _edit_points[1] = D3DXVECTOR3(0.5f, 0.2f, 0.0f);
...
bool MoveModelSpline::setOnlinePoint(size_t idx, float x, float y)
{
   D3DXVECTOR3 p(x,y,0);
   size_t i = idx * 2;
...
bool MoveModelSpline::setOfflinePoint(size_t idx, float x, float y)
{
   D3DXVECTOR3 p(x,y,0);
   size_t i = idx * 2 + 1;
   if (_edit_points.size() <= i) { return false; }
...
bool MoveModelSpline::replace(size_t idx, const D3DXVECTOR3& v)
{
...
bool MoveModelSpline::transform(const D3DXMATRIX& mat)
{
...
bool MoveModelSpline::insert(size_t idx, const D3DXVECTOR3& v)
{
   size_t beg,end;
   ...
   if (! MoveModel::insert(idx, points[1])) { return false; }
   if (! MoveModel::insert(idx, points[0])) {
      MoveModel::erase(idx);
      return false;
   }
...
bool MoveModelSpline::erase(size_t idx)
{
...

   {
      ...
      if (! MoveModel::erase(idx)) { return false; }
      if (! MoveModel::erase(idx)) {
         MoveModel::insert(idx, tmp);
         return false;
      }
   }
...
void MoveModelSpline::calcPlayPoints(size_t idx)
{
   if (idx < 2) {
      calcPlayPoints(0, 2);
   } else if ((idx & 1) == 0) {
...
void MoveModelSpline::calcPlayPoints(size_t beg, size_t end)
{
